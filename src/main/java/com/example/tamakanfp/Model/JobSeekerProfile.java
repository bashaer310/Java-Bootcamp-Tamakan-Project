...
public class JobSeekerProfile {
    ...
    //@NotEmpty(message = "Resume id must not be empty")
    //@Column(columnDefinition = "Long not null")
    //private long resumeId;

    //@NotEmpty(message = "Resume name must not be empty")
    @Column(columnDefinition = "varchar(255)")
    private String resumeName;

    //@NotEmpty(message = "Resume type must not be empty")
    @Column(columnDefinition = "varchar(255)")
    private String resumeType;

    @Lob //to convert PDF file to byte(binary data) and save it in DB
    //@NotEmpty(message = "Resume Data must not be empty")
